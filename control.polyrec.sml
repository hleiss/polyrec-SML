(* File control.polyrec.sml, a modification (by Hans Leiss, June 2023) of 
 *
 * base/compiler/Toplevel/main/control.sml
 * COPYRIGHT (c) 2019 The Fellowship of SML/NJ (http://www.smlnj.org)
 *
 * This modification
 *
 *  - adds val usemono,usepoly to Control:CONTROL 
 *  - adds val oldchecker,viewsemi,showsemi to Elab:ELABCONTROL
 *
 * to let the user switch between monomorphic and polymorphic recursion
 * by setting Elab.oldchecker (via usemono,usepoly) and inspect the solving
 * of semiunification problems during type checking (by raising the value of
 *
 *   val viewsemi : int ref
 *
 * beyond the value of the Var, Rule, Valrec cases of
 *
 *   val showsemi : {Var, Rule, Valrec, Step : int ref}
 * 
 * to be inspected. The type checker shows the semiunification problem of
 * a val rec declaration, if !viewsemi > !(# Valrec showsemi) etc., asks
 * the user whether to show steps of the constraint solving, and modifies
 * (# Step showsemi) depending on the user's answer.)
 *)
...
structure Control : CONTROL =
  struct
    ...
    structure Elab : ELAB_CONTROL = ElabControl
      (* including showsemi, viewsemi of TypecheckControl *)
    ...
    fun usepoly () = ((Elab.oldchecker:= false); primaryPrompt := "+ ")  
    fun usemono () = ((Elab.oldchecker:= true);  primaryPrompt := "- ") 
